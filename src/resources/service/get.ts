...
export class Get extends APIResource {
   ...
   * configuration endpoint of the service where the service that supports OpenID
   * Connect and
   * [OpenID Connect Discovery 1.0](https://openid.net/specs/openid-connect-discovery-1_0.html)
   * must expose its configuration information in a JSON format. Details about the
   ...
   * ```ts
   * const response =
   *   await client.service.get.getServiceConfiguration(
   *     'serviceId',
   *   );
   * ```
   */
  getServiceConfiguration(
    serviceID: string,
    query: GetGetServiceConfigurationParams | null | undefined = {},
    options?: RequestOptions,
  ): APIPromise<unknown> {
    return this._client.get(path`/api/${serviceID}/service/configuration`, { query, ...options });
  }

  /**
   * Get a list of services.
   *
   * If the access token can only view or modify clients underneath a service, but
   * does not have access to view that service directly, a limited view of the
   * service will be returned. Otherwise, all properties of the service are returned.
   *
   * If the access token is an administrative token, this returns a list of all
   * services on the Authlete instance. Otherwise, all services that the access token
   * can view, even in a limited fashion, are returned.
   *
   * @example
   * ```ts
   * const response = await client.service.get.listServices();
   * ```
   */
  listServices(
    query: GetListServicesParams | null | undefined = {},
    options?: RequestOptions,
  ): APIPromise<GetListServicesResponse> {
    return this._client.get('/api/service/get/list', { query, ...options });
  }

  /**
   * Get a service.
   *
   * If the access token can only view or modify clients underneath this service, but
   * does not have access to view this service directly, a limited view of the
   * service will be returned.
   *
   * @example
   * ```ts
   * const service = await client.service.get.retrieveService(
   *   'serviceId',
   * );
   * ```
   */
  retrieveService(serviceID: string, options?: RequestOptions): APIPromise<Service> {
    return this._client.get(path`/api/${serviceID}/service/get`, options);
  }
}
...
export interface Service {
  ...
  /**
   * The service ID used in Authlete API calls. The value of this property is
  ...
  /**
   * The time at which this service was last modified. The value is represented as
   * milliseconds since the UNIX epoch (1970-01-01).
   */
  modifiedAt?: number;
  ...
  /**
   * The sequential number of the service. The value of this property is assigned by
   * Authlete.
   */
  number?: number;
   ...
   * supports.
   *
   * The value of this property is used as `acr_values_supported` property in the
   * [OpenID Provider Metadata](https://openid.net/specs/openid-connect-discovery-1_0.html#ProviderMetadata).
   */
  supportedAcrs?: Array<string>;
   ...
   * does not work.
   */
  trustAnchors?: Array<Service.TrustAnchor>;
...
/**
 * An object representing OpenID Provider configuration information. See
 * [OpenID Provider Metadata](https://openid.net/specs/openid-connect-discovery-1_0.html#ProviderMetadata)
 * and
 ...
 * for more details.
 */
export type GetGetServiceConfigurationResponse = unknown;

export interface GetListServicesResponse {
  /**
   * Start index (inclusive) of the result set. The default value is 0. Must not be a
   * negative number.
   */
  end?: number;
...
export interface GetGetServiceConfigurationParams {
  /**
   * Get the JSON Patch
...
export interface GetListServicesParams {
  /**
   * End index (exclusive) of the result set. The default value is 5. Must not be a
   * negative number.
   */
  end?: number;
