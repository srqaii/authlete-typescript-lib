...
export class Get extends APIResource {
   ...
   * @example
   * ```ts
   * const client = await client.client.get.retrieve(
   *   'clientId',
   *   { serviceId: 'serviceId' },
   * );
   * ```
   */
  retrieve(clientID: string, params: GetRetrieveParams, options?: RequestOptions): APIPromise<Client> {
    const { serviceId } = params;
    return this._client.get(path`/api/${serviceId}/client/get/${clientID}`, options);
  }

  /**
   * Get a list of clients on a service.
   *
   * If the access token can view a full service (including an admin), all clients
   * within the service are returned. Otherwise, only clients that the access token
   * can view within the service are returned.
   *
   * - ViewClient: []
   *
   * @example
   * ```ts
   * const gets = await client.client.get.list('serviceId');
   * ```
   */
  list(
    serviceID: string,
    ...
    return this._client.get(path`/api/${serviceID}/client/get/list`, { query, ...options });
  }
}
...
export interface Client {
   ...
   * value of this property is assigned by Authlete.
   *
   * Note that Authlete issues a client secret even to a "public" client application,
   * but the client application should not use the client secret unless it changes
   * its client type to "confidential". That is, a public client application should
   * behave as if it had not been issued a client secret. To be specific, a token
   * request from a public client of Authlete should not come along with a client
   * secret although
  ...
  /**
   * The flag to indicate whether this client has been registered dynamically. For
   * more details, see [RFC 7591](https://datatracker.ietf.org/doc/html/rfc7591).
   */
  ...
  /**
   * The time at which this client was last modified. The value is represented as
   * milliseconds since the UNIX epoch (1970-01-01).
   */
  modifiedAt?: number;
  ...
  /**
   * The sequential number of the service of the client application. The value of
 ...
 *   to encrypt id_tokens
 */
export type JweEnc = 'A128CBC_HS256' | 'A192CBC_HS384' | 'A256CBC_HS512' | 'A128GCM' | 'A192GCM' | 'A256GCM';
...
export interface GetListResponse {
  ...
  /**
   * Total number of clients that belong to the service. This doesn't mean the number
   * of clients contained in the response.
   */
  totalCount?: number;
}
...
export interface GetListParams {
   ...
   * parameter is not set to `null`, client application of the specified developer
   * are returned. Otherwise, all client applications that belong to the service are
   * returned.
   */
  developer?: string;
